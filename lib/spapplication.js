...
function (_EventsDispatcher) {
  ...
  _createClass(VisualElement, [{
    ...
    value: function removeAllClasses() {
      ...
      for (var i = 0; i < classListCollectionLength; i++) {
        instance$2.removeclass(this, this.classListCollection[i]);
      }
...
var getUrl = function getUrl(definition, fileType) {
  var extension = fileType === JS_FILE_TYPE ? JS_FILE_TYPE : CSS_FILE_TYPE; //const baseURL = `${MODULE_BUILD_PATH}/${definition.moduleName}`;

  var baseURL = "".concat(definition.basePath, "/").concat(definition.moduleName);
...
function (_VisualDivElement) {
   ...
   * @property {number} dockWindowHeight
   */
  function WindowContainer(className, session, dockWindowHeight) {
    var _this;
    ...
    _this.dockWindowHeight = dockWindowHeight;
  ...
  _createClass(WindowContainer, [{
    ...
    value: function createChildren() {
      ...
      this.setMinWidthAndHeight(this.definition.minWidth, this.definition.minHeight); // const width = this.definition.width <= 0 ? window.screen.width : this.definition.width;
      // const height = this.definition.height <= 0 ? window.screen.height : this.definition.height;

      var width = this.definition.width <= 0 ? window.innerWidth : this.definition.width;
      var height = this.definition.height <= 0 ? window.innerHeight - this.dockWindowHeight : this.definition.height;
...
function () {
  ...
  _createClass(SinglePagedWindowManager, [{
    ...
    value: function addNewWindow(windowRequest) {
      ...
      var window = new WindowContainer('single-paged-window', session, _taskWindowContainer.height);
      ...
      window.setPosition(0, _taskWindowContainer.height);
...
function () {
  ...
  _createClass(ModuleTemplate, [{
    ...
    key: "setHeaderContainerWrapper",
    value: function setHeaderContainerWrapper(wrapper) {
      var styleName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      this.template.wrappers.header.obj = wrapper;

      if (!instance.isEmpty(styleName)) {
        this.template.wrappers.header.className = styleName;
      }
    }
  }, {
    key: "setAppContainerWrapper",
    value: function setAppContainerWrapper(wrapper) {
      var styleName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      this.template.wrappers.app.obj = wrapper;

      if (!instance.isEmpty(styleName)) {
        this.template.wrappers.app.className = styleName;
      }
    }
  }, {
    key: "setFooterContainerWrapper",
    value: function setFooterContainerWrapper(wrapper) {
      var styleName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      this.template.wrappers.footer.obj = wrapper;

      if (!instance.isEmpty(styleName)) {
        this.template.wrappers.footer.className = styleName;
      }
    }
  }, {
    key: "setHeaderModel",
    value: function setHeaderModel(model) {
      this.template.models.header = model;
    }
  }, {
    key: "setAppModel",
    value: function setAppModel(model) {
      this.template.models.app = model;
    }
  }, {
    key: "setFooterModel",
    value: function setFooterModel(model) {
      this.template.models.footer = model;
    }
  }, {
    key: "templateFactory",
    get: function get() {
...
export { instance$2 as DOMService, instance$3 as DialogManager, instance$8 as DragWindowService, instance$4 as DropDownManager, ElementManager, EventsDispatcher, InterWindowCommunication, ModuleLoader, ModuleTemplate, instance$7 as NumberUtils, instance$9 as ResizeWindowService, SVGService, instance$d as SinglePagedApplicationManager, SinglePagedApplicationWindowContainer, SinglePagedDialogWindowContainer, SinglePagedDropDownWindowContainer, SinglePagedInteractionWindowContainer, SinglePagedNotificationWindowContainer, SinglePagedPromotionWindowContainer, instance$c as SinglePagedTaskManager, SinglePagedTaskWindowContainer, SinglePagedTooltipWindowContainer, instance$b as SinglePagedWindowManager, instance$5 as ToolTipManager, instance$6 as UUIDUtils, instance as Utils, VisualBodyElement, VisualDivElement, VisualElement, VisualElementEvent, VisualMouseEvent, VisualSpanElement, WindowAppContainer, WindowAppContainerWrapper, WindowAppModel, WindowCommunication, WindowContainer, WindowContainerWrapper, WindowDefinition, WindowFooterContainer, WindowFooterContainerWrapper, WindowFooterModel, WindowHeaderContainer, WindowHeaderContainerWrapper, WindowHeaderModel, WindowModel, WindowRequest, WindowRequestDestination, WindowRequestInteractionSource, WindowSession, instance$a as WindowSessionManager };
//# sourceMappingURL=spapplication.js.map
